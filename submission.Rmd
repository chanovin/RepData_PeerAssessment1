...
**The mean number of steps each day is `r meandaystep`, and the median number of steps per day is `r mediandaystep`.**
...
**We see that the most active time of day for this user is typically `r mostactivetime`.**
...
**The data is missing `r numNAs` observations.**
...
**We find that the new mean steps per day is `r fixmeandaystep` compared with the `r meandaystep` from the raw data. The new median is `r fixmediandaystep` versus `r mediandaystep`. The step counts increase, naturally, since we're adding steps that were omitted as a result of the NA values.**
